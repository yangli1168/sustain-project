...
public class CaptchaGenerator {
	...
	public void outputImage(int w, int h, OutputStream os, String code) throws IOException {
		...
		Font font = null;
		try {
			InputStream is = this.getClass().getResourceAsStream("/ALGER.TTF");
	...
	public BufferedImage getGeneratedImage(int w, int h, OutputStream os, String code) throws IOException {
		int verifySize = code.length();
		BufferedImage image = new BufferedImage(w, h, BufferedImage.TYPE_INT_RGB);
		Random rand = new Random();
		Graphics2D g2 = image.createGraphics();
	...
	public static void main(String[] args) throws IOException {
		File dir = new File("/verifies"); //工程所在根目录
		
		CaptchaGenerator util = new CaptchaGenerator();
		
		int w = 200, h = 80;
		for (int i = 0; i < 3; i++) {
			String verifyCode = util.generateVerifyCode(4);
			System.out.println(verifyCode);
			File file = new File(dir, verifyCode + ".jpg");
			util.outputImage(w, h, file, verifyCode);
		}
	}
}
