...
public class CommonCacheService extends AbstractCacheService {
    ...
    public String getNextSeq(String seqDescribe, String prefix) throws CommonException {
        try {
            ...
            if (pool != null) {
                try (Jedis jedis = pool.getResource()) {
                    ...
                    // 日期格式化
                    String cast = DateUtils.getFormatDateTime(System.currentTimeMillis(),
                            DateUtils.yyyyMMddFormat_NOSEPARATOR);
                    // 组合key
                    String seqKey = "sequence:" + seqDescribe + ":" + cast;
                    if (null == jedis.get(seqKey)) {
                        jedis.set(seqKey, 1 + "");
                        next.append(prefix).append(cast).append(String.format("%04d", 1));
                        // 设置过期
                        jedis.expire(seqKey, 36 * 60 * 60);
                    } else {
                        Long incr = jedis.incr(seqKey);
                        next.append(prefix).append(cast).append(String.format("%04d", incr));
                    }
                }
            }
            ...
            logger.error("Fail to generate next val", e);
            throw new CommonException("内部错误");
        }
    }
}
