 ...
 * 第18位为校验码。检验码分别是0-10共11个数字，当检验码为“10”时，为了保证公民身份证号码18位，所以用“X”表示。虽然校验码为“X”不能更换，
 * 但若需全用数字表示，只需将18位公民身份号码转换成15位居民身份证号码，去掉第7至8位和最后1位3个数码。
 * 当今的身份证号码有15位和18位之分。1985年我国实行居民身份证制度，当时签发的身份证号码是15位的，1999年签发的身份证由于年份的扩展（
 * 由两位变为四位）和末尾加了效验码，就成了18位。 （1）前1、2位数字表示：所在省份的代码； （2）第3、4位数字表示：所在城市的代码；
 * （3）第5、6位数字表示：所在区县的代码； （4）第7~14位数字表示：出生年、月、日； （5）第15、16位数字表示：所在地的派出所的代码；
...
public class IDCardUtil {
	...
	public static boolean IDCardValidate(String IDStr) throws CommonException {
		if (IDStr == null) {
			throw new CommonException("身份证号码为空");
		}
		String Ai = "";
		// 判断号码的长度 15位或18位
		if (IDStr.length() != 15 && IDStr.length() != 18) {
			throw new CommonException("身份证号码长度非法");
		}
		...
		if (isNumeric(Ai) == false) {
			throw new CommonException("身份证号码除最后一位外必须为数字");
		}
		...
		if (isDate(strYear + "-" + strMonth + "-" + strDay) == false) {
			throw new CommonException("身份证出生日期无效。");
		}
		...
		if (areacode.get(Ai.substring(0, 2)) == null) {
			throw new CommonException("身份证地区编码错误。");
		}

		if (isVarifyCode(Ai, IDStr) == false) {
			throw new CommonException("身份证校验码非法");
		}
